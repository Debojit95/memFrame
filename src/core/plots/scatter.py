...
class ScatterPlotCore:
    """
    Core scatter plotting engine.

    This class fetches data from the active table and delegates chart creation
    ...
    async def _fetch_dataframe(
        ...
        if columns is None:
            ...
            if not selected:
                # If we cannot safely infer referenced columns, fetch full table.
                selected = available_columns
    ...
    async def scatter(
        ...
        **kwargs: Any,
        ...
        """
        Build a scatter plot using plotly.express.scatter.

        The method accepts the same parameters as px.scatter except data_frame,
        which is automatically resolved from the active memFrame context.
        """
        self._ensure_plotly()

        if "data_frame" in kwargs:
            raise ValueError(
                "Do not pass 'data_frame' to ops.scatter(); it is derived from the active context."
            )
