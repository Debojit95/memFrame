...
def async_to_sync(func: Callable[P, Awaitable[R]]) -> Callable[P, R]:
    ...
    - Handles running event loops by offloading to a worker thread
    ...
    def wrapper(*args: P.args, **kwargs: P.kwargs) -> R:
        ...
        # Running loop detected: keep the public sync API blocking, but run
        # the coroutine on a worker thread with its own event loop.
        def runner():
            return asyncio.run(func(*args, **kwargs))
