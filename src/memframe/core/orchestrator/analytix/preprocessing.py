...
from memframe.core.analytix.preprocessing import PreprocessingOps
from memframe.cache import record_call


class PreprocessingOrchestrator:
    """Orchestrator for feature engineering preprocessing operations."""
    ...
    async def _ensure_ops(self) -> PreprocessingOps:
        if self._Preprocessing_ops is None:
            await self._ops_parent._ensure_adapter()
            self._Preprocessing_ops = PreprocessingOps(self._ops_parent._adapter)
        return self._Preprocessing_ops
...
PreprocessAccessor = PreprocessingOrchestrator
