...
class GeneralTableOps:
    ...
    Backend-agnostic logic lives here. The handful of places where DuckDB /
    PostgreSQL / ClickHouse genuinely differ are delegated to small overridable
    "hook" methods (see the ``_limit_offset``, ``_numeric_stat_exprs``,
    ``_cast_expr``, ... family). Concrete backend subclasses in
    ``duckdb.py`` / ``postgres.py`` / ``clickhouse.py`` override only the hooks
    that differ; ClickHouse additionally overrides the few methods with a
    wholly different algorithm (``dataframe_insert`` / ``dataframe_update`` /
    ``dataframe_set_index`` / ``_sql_type_for_series``).
    ...
    def _sql_type_for_series(self, series: pd.Series) -> str:
        """PostgreSQL/DuckDB type mapping for a pandas Series.

        ClickHouse's version (which wraps nullable columns in ``Nullable(...)``)
        lives in ``clickhouse.py``.
        """
        dtype = series.dtype
    ...
    async def _save_dataframe_as_table(
        ...
        # ClickHouse needs an ENGINE; the default (empty) is correct for PG/DuckDB.
        create_sql += self._create_table_suffix()
        ...
        if not df_to_store.empty:
            ...
            await self._insert_rows(qualified_new, rows, list(df_to_store.columns))
    ...
    # ------------------------------------------------------------------
    # Backend-divergence hooks (DuckDB-flavoured defaults)
    # ------------------------------------------------------------------
    def _limit_offset(self, limit: int, offset: int) -> str:
        """SQL suffix for LIMIT/OFFSET; ClickHouse reverses the order."""
    ...
    def _numeric_stat_exprs(self, col: str) -> Dict[str, str]:
        """Percentile/stddev SQL fragments for describe / numeric_basic_summary."""
        return {
            "q25": f'QUANTILE_CONT("{col}", 0.25)',
            "median": f'QUANTILE_CONT("{col}", 0.5)',
            "q75": f'QUANTILE_CONT("{col}", 0.75)',
    ...
    async def _insert_rows(self, qualified: str, rows: List[List[Any]], columns: List[str]) -> None:
        quoted_cols = ", ".join(self.db.quote_identifier(c) for c in columns)
        placeholders = ", ".join(self.db.placeholder(i + 1) for i in range(len(columns)))
        insert_sql = f"INSERT INTO {qualified} ({quoted_cols}) VALUES ({placeholders})"
        for values in rows:
    ...
    def _cast_expr(self, col_safe: str, target_type: str, source_type: str, requested_dtype: str) -> str:
        return f'CAST("{col_safe}" AS {target_type}) AS "{col_safe}"'
    ...
    async def dataframe_tail(self, table: str, schema: str, n: int = 10, columns: Optional[List[str]] = None, **kwargs) -> Dict[str, Any]:
        try:
            ...
            query = f"SELECT {column_clause} FROM {qualified} {self._limit_offset(n, offset)}"
    ...
    async def dataframe_sample(self, table: str, schema: str, n: int = 10, columns: Optional[List[str]] = None, random_state: Optional[int] = None, **kwargs) -> Dict[str, Any]:
        try:
            ...
            await self._apply_random_seed(random_state)
    ...
    async def dataframe_describe(self, table: str, schema: str, columns: Optional[List[str]] = None, **kwargs) -> Dict[str, Any]:
        try:
            ...
            for col in sanitized:
                exprs = self._numeric_stat_exprs(col)
                q25_expr = exprs["q25"]
                median_expr = exprs["median"]
                q75_expr = exprs["q75"]
                std_expr = exprs["std"]
    ...
    async def dataframe_null_analysis(self, table: str, schema: str, columns: Optional[List[str]] = None, **kwargs) -> Dict[str, Any]:
        try:
            ...
            actual_columns = await self._list_table_columns(schema, table)
    ...
    async def numeric_basic_summary(
        ...
        try:
            ...
            exprs = self._numeric_stat_exprs(safe_col)
            q25 = exprs["q25"]
            median_expr = exprs["median"]
            q75 = exprs["q75"]
            std_expr = exprs["std"]
                ...
                async def iterator():
                    ...
                    while True:
                        rows = await self._fetch(
                            f"SELECT {column_clause} FROM {qualified} {self._limit_offset(chunk_size, offset)}"
                        )
                        if not rows:
                            break
    ...
    async def dataframe_astype(self, table: str, schema: str, dtype_map: Dict[str, str], **kwargs) -> Dict[str, Any]:
        try:
            ...
            translation = self._dtype_translation_map()
            ...
            for col, dtype in dtype_map.items():
                ...
                if dtype_lower not in translation:
                    return self._error_response(f"Unsupported dtype '{dtype}'")

                normalized_map[col] = translation[dtype_lower]
            ...
            for col, dtype in normalized_map.items():
                ...
                select_parts.append(
                    self._cast_expr(col_safe, dtype, column_types[col].lower(), requested_dtype_map[col])
                )
    ...
    # ------------------------------------------------------------------
    # Iterator Methods (async generators)
    # ------------------------------------------------------------------
                ...
                for col in cols:
                    ...
                    async def column_generator():
                        async for row in self.db.fetch_iter(
                            ...
                            yield self._row_value(row, col_safe, [col_safe])
        ...
        try:
            ...
            async def tuple_generator():
                ...
                async for row in self.db.fetch_iter(
                    ...
                    values = self._row_to_list(row, columns)
