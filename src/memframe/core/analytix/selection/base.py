...
Backend-specific differences are isolated in small overridable hooks so the
shared logic lives once in DataSelectionOps (base.py). DuckDB-flavoured SQL is
the default; Postgres/ClickHouse subclasses override only the divergent hooks.
"""
...
class DataSelectionOps:
    ...
    # ------------------------------------------------------------------
    # Backend hooks (DuckDB defaults; override per adapter)
    # ------------------------------------------------------------------
    def _list_columns_query(self, qualified: str, schema: str, table: str) -> Tuple[str, List[Any]]:
        # ponytail: DuckDB default; Postgres/ClickHouse override
        return (f"PRAGMA table_info({qualified})", [])
    ...
    def _column_name(self, row: Any) -> str:
        return self._row_get(row, "name", 1)

    def _column_type(self, row: Any) -> str:
        return self._row_get(row, "type", 2)

    def _serialize_asof_where(self, normalized: Any) -> Any:
        # ponytail: DuckDB/ClickHouse serialize date/datetime to ISO strings;
        # Postgres overrides this to pass values through unchanged.
