...
class DataMergeOps:
    ...
    def _unsupported_backend_error(self) -> NotImplementedError:
        return NotImplementedError(
            f"Unsupported database backend for merge operation: {self.db.__class__.__name__}"
        )
    ...
    async def merge(
        ...
        try:
            if isinstance(self.db, PostgresAdapter) or isinstance(self.db, DuckDBAdapter):
                ...
                create_sql = f"""
                    CREATE TABLE {self.db.quote_identifier(schema)}.{self.db.quote_identifier(new_table)} AS
                    SELECT {select_sql}
                    FROM {left_q} l
                    {join_sql} {right_q} r
                ...
                # -----------------------------
                # JOIN CONDITION (ClickHouse-aware auto-cast)
                # -----------------------------
                if how == "cross":
                    join_sql = "CROSS JOIN"
                    condition_sql = ""
                    ...
                    for l, r in zip(left_keys, right_keys):
                        ...
                        # 🔥 AUTO CAST LOGIC (ClickHouse-aware)
                        # ClickHouse uses DateTime/DateTime64 for timestamps, Date/Date32 for dates
                        ...
                        # "DATE" must NOT be part of "DATETIME" to count as a date-only type
                ...
                # -----------------------------
                # CREATE TABLE (ClickHouse: ENGINE = MergeTree)
                # -----------------------------
                new_table = await self._resolve_output_table_name(
                    base_table=left_table,
                    schema=schema,
                ...
                create_sql = f"""
                    CREATE TABLE {self.db.quote_identifier(schema)}.{self.db.quote_identifier(new_table)}
                    ENGINE = MergeTree()
                    ORDER BY tuple()
                    AS
                    SELECT {select_sql}
