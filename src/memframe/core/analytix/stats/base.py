...
class DataStatsOps:
    ...
    def _quantile_expr(self, c: str, qt) -> str:
        return f'QUANTILE_CONT({c}, {qt})'

    def _double_cast(self, c: str) -> str:
        return f'CAST("{c}" AS DOUBLE)'
    ...
    def _epoch_expr(self, c: str) -> str:
        return f'epoch(CAST({c} AS TIMESTAMP))'
    ...
    # ------------------------------------------------------------------
    # Shared stats operations (hoisted verbatim; call the hooks above)
    # ------------------------------------------------------------------
    ...
    async def numeric_prod(self, table: str, schema: str, column: str) -> Dict[str, Any]:
        try:
            q = self._qualified_table(table, schema)
            c = SQLIdentifierSanitizer.sanitize(column)
            prod_val = await self._fetchval(f'SELECT {self._fn_name("EXP")}({self._fn_name("SUM")}({self._fn_name("LN")}("{c}"))) FROM {q} WHERE "{c}" > 0')
    ...
    async def numeric_std(self, table: str, schema: str, column: str) -> Dict[str, Any]:
        try:
            q = self._qualified_table(table, schema)
            c = SQLIdentifierSanitizer.sanitize(column)
            std_val = await self._fetchval(f'SELECT {self._fn_name("STDDEV_POP")}("{c}") FROM {q} WHERE "{c}" IS NOT NULL')
    ...
    async def numeric_var(self, table: str, schema: str, column: str) -> Dict[str, Any]:
        try:
            q = self._qualified_table(table, schema)
            c = SQLIdentifierSanitizer.sanitize(column)
            var_val = await self._fetchval(f'SELECT {self._fn_name("VAR_POP")}("{c}") FROM {q} WHERE "{c}" IS NOT NULL')
    ...
    async def numeric_sem(self, table: str, schema: str, column: str) -> Dict[str, Any]:
        try:
            q = self._qualified_table(table, schema)
            c = SQLIdentifierSanitizer.sanitize(column)
            val = await self._fetchval(f'SELECT {self._fn_name("STDDEV_SAMP")}("{c}") / {self._fn_name("SQRT")}({self._fn_name("COUNT")}("{c}")) FROM {q} WHERE "{c}" IS NOT NULL')
    ...
    async def numeric_mad(self, table: str, schema: str, column: str) -> Dict[str, Any]:
        try:
            q = self._qualified_table(table, schema)
            c = SQLIdentifierSanitizer.sanitize(column)
            val = await self._fetchval(f'SELECT {self._fn_name("AVG")}({self._fn_name("ABS")}("{c}" - sub.avg_val)) FROM {q}, (SELECT {self._fn_name("AVG")}("{c}") AS avg_val FROM {q} WHERE "{c}" IS NOT NULL) AS sub WHERE "{c}" IS NOT NULL')
    ...
    async def numeric_coefficient_of_variation(self, table: str, schema: str, column: str) -> Dict[str, Any]:
        try:
            q = self._qualified_table(table, schema)
            c = SQLIdentifierSanitizer.sanitize(column)
            val = await self._fetchval(f'SELECT {self._fn_name("STDDEV_POP")}("{c}") / {self._fn_name("AVG")}("{c}") FROM {q} WHERE "{c}" IS NOT NULL')
    ...
    async def numeric_median(self, table: str, schema: str, column: str) -> Dict[str, Any]:
        try:
            q = self._qualified_table(table, schema)
            c = SQLIdentifierSanitizer.sanitize(column)
            median_sql = self._median_expr(f'"{c}"')
    ...
    async def numeric_iqr(self, table: str, schema: str, column: str) -> Dict[str, Any]:
        try:
            q = self._qualified_table(table, schema)
            c = SQLIdentifierSanitizer.sanitize(column)
            q3_expr = self._quantile_expr(f'"{c}"', 0.75)
            q1_expr = self._quantile_expr(f'"{c}"', 0.25)
    ...
    async def numeric_quantile(self, table: str, schema: str, column: str, quantiles: List[float]=None) -> Dict[str, Any]:
        try:
            ...
            c = SQLIdentifierSanitizer.sanitize(column)
            qcol = f'"{c}"'
            parts = ', '.join((f'{self._quantile_expr(qcol, qt)} AS p_{int(qt * 100)}' for qt in quantiles))
    ...
    async def numeric_outliers_iqr(self, table: str, schema: str, column: str) -> Dict[str, Any]:
        try:
            q = self._qualified_table(table, schema)
            c = SQLIdentifierSanitizer.sanitize(column)
            col_expr = self._double_cast(c)
            q1_expr = self._quantile_expr(col_expr, 0.25)
            q3_expr = self._quantile_expr(col_expr, 0.75)
    ...
    async def numeric_autocorr(self, table: str, schema: str, column: str, lag: int=1) -> Dict[str, Any]:
        try:
            ...
            c = SQLIdentifierSanitizer.sanitize(column)
            qcol = f'"{c}"'
            sql = f'\n                    WITH lagged AS (\n                        SELECT "{c}" AS cur, {self._lag_fn(qcol, lag)} OVER (ORDER BY (SELECT NULL)) AS prev\n                        FROM {q}\n                    )\n                    SELECT {self._fn_name("CORR")}(cur, prev) FROM lagged WHERE cur IS NOT NULL AND prev IS NOT NULL\n                '
            val = await self._fetchval(sql)
    ...
    async def _multi_column_assoc_matrix(self, table: str, schema: str, columns: List[str], agg_fn: str, backend=None, data_id: Optional[str]=None, new_table: Optional[str]=None) -> Dict[str, Any]:
        ...
        try:
            ...
            def _cast(c: str) -> str:
                return self._text_to_double(c)
            alias = {c: f'a{idx}' for idx, c in enumerate(cols)}
            pairs = [(i, j) for i in range(n) for j in range(i, n)]
            B = self._assoc_batch_cap(len(pairs))
    ...
    async def categorical_multi_column_crosstab(self, table: str, schema: str, columns: List[str], backend=None, data_id: Optional[str]=None, new_table: Optional[str]=None) -> Dict[str, Any]:
        try:
            ...
            combined = self._concat_pipe([f'"{c}"' for c in qcols])
    ...
    async def datetime_mean(self, table: str, schema: str, column: str) -> Dict[str, Any]:
        try:
            ...
            epoch_expr = self._epoch_expr(f'"{c}"')
            from_epoch_expr = self._from_epoch('value_epoch')
            if is_date_only:
                from_epoch_expr = self._date_from_epoch(from_epoch_expr)
    ...
    async def datetime_median(self, table: str, schema: str, column: str) -> Dict[str, Any]:
        try:
            ...
            is_date_only = 'date' in detected_type and 'time' not in detected_type
            epoch_expr = self._epoch_expr(f'"{c}"')
            median_epoch_expr = self._median_expr(epoch_expr)
            from_epoch_expr = self._from_epoch('value_epoch')
            if is_date_only:
    ...
    async def datetime_delta_stats(self, table: str, schema: str, column: str) -> Dict[str, Any]:
        try:
            q = self._qualified_table(table, schema)
            c = SQLIdentifierSanitizer.sanitize(column)
            epoch_b = self._epoch_expr(f'b."{c}"')
            epoch_a = self._epoch_expr(f'a."{c}"')
            diff_expr = f'{epoch_b} - {epoch_a}'
            median_expr = self._median_expr('d')
