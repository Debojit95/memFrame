...
class Uploader:
    ...
    async def _create_table_from_parquet(self, table_name: str, file_path: str) -> int:
        ...
        if self._backend.backend == Backend.DUCKDB:
            ...
            self._backend._conn.register("parquet_temp", full_table)
            self._backend._conn.execute(f"INSERT INTO {final_table} SELECT * FROM parquet_temp")
            self._backend._conn.unregister("parquet_temp")
        elif self._backend.backend == Backend.POSTGRES:
            final_table = f'{schema_name}."{table_name}"'
            await self._create_final_table_direct(final_table, columns, schema)
    ...
    async def _insert_arrow_table_postgres(self, final_table: str, arrow_table: pa.Table, columns: List[str]) -> None:
        ...
        with io.BytesIO() as buf:
            ...
            buf.seek(0)
            await self._backend._conn.copy_to_table(
                self._backend._split_qualified_table_name(final_table)[1],
                source=buf,
                columns=columns,
                schema_name=self._backend.upload_schema,
                format="csv",
