...
def get_generated_col(result: Any, fallback: str) -> str:
    ...
    if isinstance(result, pd.DataFrame):
        for col in result.columns:
            if str(col).startswith("cleaned_"):
                return str(col)
        return fallback
    if isinstance(result, dict):
...
def get_materialized_table_columns(ctx: Any, result: Any) -> List[str]:
    """Return the columns materialized by an operation."""
    if isinstance(result, pd.DataFrame):
        return list(result.columns)
    raise AssertionError(f"Expected a DataFrame result, got: {type(result)}: {result}")
...
class TestCleaningOperations:
    ...
    def test_fillna_materialized_table_keeps_only_involved_and_generated_columns(
        self, uploaded_ctx, sample_df, backend_config
    ):
        result = uploaded_ctx.fillna(column="salary", method="median")
        out_col = get_generated_col(result, "salary")

        assert get_materialized_table_columns(uploaded_ctx, result) == ["salary", out_col]
    ...
    def test_groupby_fillna_materialized_table_keeps_only_involved_and_generated_columns(
        self, uploaded_ctx, sample_df, backend_config
    ):
        result = uploaded_ctx.groupby_fillna(
            column="salary", group_cols=["department"], method="max"
        )
        out_col = get_generated_col(result, "salary")

        assert get_materialized_table_columns(uploaded_ctx, result) == [
            "department",
            "salary",
            out_col,
        ]
    ...
    def test_comprehensive_numeric_summary(self, uploaded_ctx, sample_df, backend_config):
        ...
        with pytest.raises(OperationError):
            uploaded_ctx.comprehensive_numeric_summary(columns=cols)
