...
class TestPiePlotOperations:
    ...
    def setup_class(self, request, save_to_file):
        ...
        if cls._save_to_file and cls._saved_results:
            ...
            pdf_path = RESULT_DIR / f"test_pie_report_{request.node.name}.pdf"
    ...
    # ------------------------------------------------------------------
    # Simple functional tests to verify pie plot works across backends
    # ------------------------------------------------------------------
    def test_pie_count(self, uploaded_ctx, backend_config):
        """Test basic count pie plot (x only)."""
        fig = uploaded_ctx.pie(names="category")
        self._record_result("pie_count", 'pie(x="category")', fig, backend_config["connection_type"])

    def test_pie_agg(self, uploaded_ctx, backend_config):
        """Test aggregation pie plot with y specified."""
        fig = uploaded_ctx.pie(names="category", values="score")
        self._record_result("pie_agg", 'pie(names="category", values="score")', fig, backend_config["connection_type"])