...
def _usage_error(message: str) -> pytest.UsageError:
    return pytest.UsageError(f"Invalid sorting DB configuration: {message}")
...
def sample_df() -> pd.DataFrame:
    """DataFrame with mixed types and some nulls for sorting tests."""
    return pd.DataFrame({
        "id": [3, 1, 2, 5, 4],
        "name": ["Charlie", "Alice", "Bob", None, "Eve"],
...
def uploaded_ctx(connected_memframe, sample_df) -> Any:
    ...
    return connected_memframe.upload_df(sample_df, filename="sorting_dataset")
...
# ----------------------------------------------------------------------
# PDF helpers (for optional reporting)
# ----------------------------------------------------------------------
...
class TestSortingOperations:
    """All sorting tests that require a backend connection."""
    ...
    def setup_class(self, request, save_to_file):
        ...
        if cls._save_to_file and cls._saved_results:
            ...
            pdf_path = RESULT_DIR / f"test_sorting_report_{request.node.name}.pdf"
    ...
    def _record_failure_from_report(self, request, error_message: str) -> None:
        frame_locals = getattr(request.node, "_sorting_failure_locals", None)
        ...
        memframe_value = None
        for name in ("res_df", "original", "result"):
        ...
        pandas_value = None
        for name in ("expected", "expected_df", "expected_series", "sample_df"):
            if name in frame_locals:
    ...
    # Single column sort
    # ------------------------------------------------------------------
    def test_sort_single_ascending(self, uploaded_ctx, sample_df, backend_config):
        result = uploaded_ctx.sort_values(by="id", ascending=True)
        res_df = get_result_df(result)
        expected = sample_df.sort_values("id").reset_index(drop=True)
        if "name" in res_df.columns and "name" in expected.columns:
            res_df["name"] = res_df["name"].fillna("")
    ...
    def test_sort_single_descending(self, uploaded_ctx, sample_df, backend_config):
        result = uploaded_ctx.sort_values(by="score", ascending=False)
        res_df = get_result_df(result)
        expected = sample_df.sort_values("score", ascending=False, na_position="last").reset_index(drop=True)
        # Fill nulls in string column to avoid mismatch
        for col in expected.select_dtypes(object).columns:
    ...
    # Multi-column sort
    # ------------------------------------------------------------------
    def test_sort_multi(self, uploaded_ctx, sample_df, backend_config):
        result = uploaded_ctx.sort_values(by=["group", "id"], ascending=[True, False])
        res_df = get_result_df(result)
        expected = sample_df.sort_values(["group", "id"], ascending=[True, False]).reset_index(drop=True)
        for col in expected.select_dtypes(object).columns:
            if col in res_df.columns:
                res_df[col] = res_df[col].fillna("")
                expected[col] = expected[col].fillna("")
    ...
    # NA position
    # ------------------------------------------------------------------
    def test_sort_na_first(self, uploaded_ctx, sample_df, backend_config):
        # Test na_position="first" on a column with nulls (name)
        result = uploaded_ctx.sort_values(by="name", ascending=True, na_position="first")
        res_df = get_result_df(result)
        expected = sample_df.sort_values("name", ascending=True, na_position="first").reset_index(drop=True)
        # The library may represent nulls as empty strings; fill both
        res_df["name"] = res_df["name"].fillna("")
        expected["name"] = expected["name"].fillna("")
        pd.testing.assert_frame_equal(
            normalize_frame(res_df),
    ...
    # ------------------------------------------------------------------
    def test_sort_columns_subset(self, uploaded_ctx, sample_df, backend_config):
        # Sort and return only selected columns
        result = uploaded_ctx.sort_values(by="id", ascending=True, columns=["name", "score"])
        res_df = get_result_df(result)
        # Core keeps sort key(s) appended to output when columns subset is requested.
        expected = sample_df.sort_values("id")[["name", "score", "id"]].reset_index(drop=True)
        # Fill nulls in name
        res_df["name"] = res_df["name"].fillna("")
        expected["name"] = expected["name"].fillna("")
        pd.testing.assert_frame_equal(
    ...
    def test_mutation_safety(self, uploaded_ctx, sample_df, backend_config):
        _ = uploaded_ctx.sort_values(by="id")
        original = get_result_df(uploaded_ctx)
        if "name" in original.columns and "name" in sample_df.columns:
            original["name"] = original["name"].fillna("")
            sample_df = sample_df.copy()
            sample_df["name"] = sample_df["name"].fillna("")
        pd.testing.assert_frame_equal(
            normalize_frame(original),
