...
def _usage_error(message: str) -> pytest.UsageError:
    return pytest.UsageError(f"Invalid groupby_stats DB configuration: {message}")
...
def sample_df() -> pd.DataFrame:
    """DataFrame with groups, numeric, and datetime columns for group-by stats."""
    return pd.DataFrame({
        "group": ["A", "A", "A", "B", "B", "B"],
...
def uploaded_ctx(connected_memframe, sample_df) -> Any:
    ...
    ctx = connected_memframe.upload_df(sample_df, filename="groupby_stats_dataset")
    return ctx


# ----------------------------------------------------------------------
# Helpers (reused from arithmetic pattern, adapted for group-by results)
# ----------------------------------------------------------------------
...
def compare_groupby_result(
    ...
    """
    Compare two DataFrames resulting from group-by aggregations,
    ignoring row/column ordering, helper columns, and dtype differences.
    """
    actual = normalize_frame(actual_df)
    expected = normalize_frame(expected_df)
    if sort_cols:
...
# Test class
# ----------------------------------------------------------------------
class TestGroupByStatsOperations:
    _save_to_file = False
    _saved_results = []
    ...
    def setup_class(self, request, save_to_file):
        ...
        if cls._save_to_file and cls._saved_results:
            ...
            pdf_path = RESULT_DIR / f"test_groupby_stats_report_{request.node.name}.pdf"
    ...
    def _record_failure_from_traceback(self, request, exc: Exception) -> None:
        ...
        memframe_value = None
        for name in ("res_df", "after_op", "original_uploaded", "merged", "result"):
        ...
        pandas_value = None
        for name in ("expected", "expected_df"):
            if name in frame_locals:
    ...
    def test_sum(self, uploaded_ctx, sample_df, backend_config):
        result = uploaded_ctx.groupby("group").sum("value")
        res_df = get_result_df(result)
        expected = sample_df.groupby("group", as_index=False)["value"].sum()

        # The library’s column may be named "value_sum" – rename expected to match
        lib_col = get_generated_col(result, "value_sum")
    ...
    def test_mean(self, uploaded_ctx, sample_df, backend_config):
        result = uploaded_ctx.groupby("group").mean("value")
        res_df = get_result_df(result)
    ...
    def test_min(self, uploaded_ctx, sample_df, backend_config):
        result = uploaded_ctx.groupby("group").min("value")
        res_df = get_result_df(result)
    ...
    def test_max(self, uploaded_ctx, sample_df, backend_config):
        result = uploaded_ctx.groupby("group").max("value")
        res_df = get_result_df(result)
    ...
    def test_count(self, uploaded_ctx, sample_df, backend_config):
        result = uploaded_ctx.groupby("group").count("value")
        res_df = get_result_df(result)
        expected = sample_df.groupby("group", as_index=False)["value"].count()

        lib_col = get_generated_col(result, "value_count")
        expected = expected.rename(columns={"value": lib_col})
    ...
    def test_median(self, uploaded_ctx, sample_df, backend_config):
        result = uploaded_ctx.groupby("group").median("value")
        res_df = get_result_df(result)
    ...
    def test_std(self, uploaded_ctx, sample_df, backend_config):
        result = uploaded_ctx.groupby("group").std("value")
        res_df = get_result_df(result)
        expected = sample_df.groupby("group", as_index=False)["value"].std(ddof=1)
        # library might use population std or sample std; adjust accordingly.
        # We'll test with ddof=1 (sample) as typical; if mismatch, try ddof=0.
    ...
    def test_var(self, uploaded_ctx, sample_df, backend_config):
        result = uploaded_ctx.groupby("group").var("value")
        res_df = get_result_df(result)
        expected = sample_df.groupby("group", as_index=False)["value"].var(ddof=1)
    ...
    def test_sem(self, uploaded_ctx, sample_df, backend_config):
        result = uploaded_ctx.groupby("group").sem("value")
        res_df = get_result_df(result)
        expected = sample_df.groupby("group", as_index=False)["value"].sem(ddof=1)
    ...
    def test_nunique(self, uploaded_ctx, sample_df, backend_config):
        result = uploaded_ctx.groupby("group").nunique("category")
        res_df = get_result_df(result)
        expected = sample_df.groupby("group", as_index=False)["category"].nunique()

        lib_col = get_generated_col(result, "category_nunique")
    ...
    def test_range(self, uploaded_ctx, sample_df, backend_config):
        result = uploaded_ctx.groupby("group").range("value")
        res_df = get_result_df(result)
        value_col = get_generated_col(result, "value_range")
    ...
    def test_product(self, uploaded_ctx, sample_df, backend_config):
        result = uploaded_ctx.groupby("group").product("value")
        res_df = get_result_df(result)
    ...
    # ------------------------------------------------------------------
    def test_agg_dict(self, uploaded_ctx, sample_df, backend_config):
        agg_dict = {
            "value": ["sum", "mean"],
            "score": ["min", "max"]
        }
        result = uploaded_ctx.groupby("group").agg(agg_dict)
        res_df = get_result_df(result)

        # Build expected with identical column names as the library uses
        # (<column>_<stat> on both sides).
        lib_cols = ["value_sum", "value_mean", "score_min", "score_max"]

        expected = sample_df.groupby("group", as_index=False).agg(
            value_sum=("value", "sum"),
    ...
    # ------------------------------------------------------------------
    # Alternate representations: multi-column agg dicts, direct vs builder
    # ------------------------------------------------------------------
    def test_agg_dict_multi_column_builder(self, uploaded_ctx, sample_df, backend_config):
        result = uploaded_ctx.groupby("group").agg({"value": ["sum"], "score": ["max"]})
        res_df = get_result_df(result)

        lib_cols = ["value_sum", "score_max"]
        expected = sample_df.groupby("group", as_index=False).agg(
            value_sum=("value", "sum"),
            score_max=("score", "max"),
        )
    ...
    def test_agg_dict_multi_column_direct(self, uploaded_ctx, sample_df, backend_config):
        result = uploaded_ctx.agg(
            group_cols=["group"], agg_dict={"value": ["sum"], "score": ["max"]}
        )
        res_df = get_result_df(result)
    ...
    # ------------------------------------------------------------------
    def test_multicolumn_groupby_builder(self, uploaded_ctx, sample_df, backend_config):
        result = uploaded_ctx.groupby("group", "category").agg({"value": ["sum", "mean"]})
        res_df = get_result_df(result)

        expected = sample_df.groupby(["group", "category"], as_index=False).agg(
            value_sum=("value", "sum"),
            value_mean=("value", "mean"),
        )

        compare_groupby_result(
            res_df, expected, group_cols=["group", "category"],
            sort_cols=["group", "category"],
        )
    ...
    def test_multicolumn_groupby_direct(self, uploaded_ctx, sample_df, backend_config):
        result = uploaded_ctx.agg(
            group_cols=["group", "category"],
            agg_dict={"value": ["count"], "score": ["min", "max"]},
        )
        res_df = get_result_df(result)
    ...
    def test_event_rate(self, uploaded_ctx, sample_df, backend_config):
        result = uploaded_ctx.groupby("group").event_rate("dt", unit="day")
        res_df = get_result_df(result)
        # expected: count / ((max_dt - min_dt).days)  but if same timestamp, days=0 -> inf? Our data has spread.
        expected = sample_df.groupby("group").apply(
            lambda g: g["dt"].count() / max((g["dt"].max() - g["dt"].min()).total_seconds() / 86400, 1e-6)
        ).reset_index(name="event_rate_day")
    ...
    def test_mutation_safety(self, uploaded_ctx, sample_df, backend_config):
        ...
        uploaded_ctx.groupby("group").sum("value")
