...
def render_fig_to_pdf_page(
    ...
    if plot_fig is not None and hasattr(plot_fig, "suptitle"):
        plot_fig.suptitle(
        ...
        if error_message:
            plot_fig.text(
                ...
                fontsize=9, color="crimson", transform=plot_fig.transFigure,
            )
        ...
        fig.text(0.5, 0.5, "No figure generated" if plot_fig is None else "Figure type not embeddable in PDF", ha="center", va="center", fontsize=14)
        pdf.savefig(fig)
        plt.close(fig)
...
# Test class
# ----------------------------------------------------------------------
class TestScatterPlotOperations:
    _save_to_file = False
    _saved_results = []
    ...
    def setup_class(self, request, save_to_file):
        ...
        if cls._save_to_file and cls._saved_results:
            ...
            pdf_path = RESULT_DIR / f"test_scatter_report_{request.node.name}.pdf"
    ...
    # ------------------------------------------------------------------
    # Simple functional tests to verify scatter plot works across backends
    # ------------------------------------------------------------------
    def test_scatter_basic(self, uploaded_ctx, backend_config):
        """Test basic scatter plot (x and y)."""
        fig = uploaded_ctx.scatter(x="score", y="salary")
        self._record_result("scatter_basic", 'scatter(x="score", y="salary")', fig, backend_config["connection_type"])

    def test_scatter_with_hue(self, uploaded_ctx, backend_config):
        """Test scatter plot with hue grouping."""
        fig = uploaded_ctx.scatter(x="score", y="salary", color="category")
        self._record_result("scatter_with_hue", 'scatter(x="score", y="salary", color="category")', fig, backend_config["connection_type"])