...
class TestLinePlotOperations:
    ...
    def setup_class(self, request, save_to_file):
        ...
        if cls._save_to_file and cls._saved_results:
            ...
            pdf_path = RESULT_DIR / f"test_line_report_{request.node.name}.pdf"
    ...
    # ------------------------------------------------------------------
    # Simple functional tests to verify line plot works across backends
    # ------------------------------------------------------------------
    def test_line_basic(self, uploaded_ctx, backend_config):
        """Test basic line plot (x and y)."""
        fig = uploaded_ctx.line(x="count_all", y="score")
        self._record_result("line_basic", 'line(x="count_all", y="score")', fig, backend_config["connection_type"])

    def test_line_agg_with_hue(self, uploaded_ctx, backend_config):
        """Test aggregation line plot with hue."""
        fig = uploaded_ctx.line(x="category", y="score", color="category2")
        self._record_result("line_color", 'x="category", y="score", color="category2"', fig, backend_config["connection_type"])