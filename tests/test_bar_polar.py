...
# Test class
# ----------------------------------------------------------------------
class TestBarPolarPlotOperations:
    _save_to_file = False
    _saved_results = []
    ...
    def setup_class(self, request, save_to_file):
        ...
        if cls._save_to_file and cls._saved_results:
            ...
            pdf_path = RESULT_DIR / f"test_bar_polar_report_{request.node.name}.pdf"
    ...
    # ------------------------------------------------------------------
    # Simple functional tests to verify bar_polar plot works across backends
    # ------------------------------------------------------------------
    def test_bar_polar_basic(self, uploaded_ctx, backend_config):
        """Test basic bar polar plot (theta only)."""
        fig = uploaded_ctx.bar_polar(theta="category")
        self._record_result("bar_polar_basic", 'bar_polar(theta="category")', fig, backend_config["connection_type"])

    def test_bar_polar_agg_with_hue(self, uploaded_ctx, backend_config):
        """Test aggregation bar polar plot with r and hue."""
        fig = uploaded_ctx.bar_polar(theta="category", r="score")
        self._record_result("bar_polar_2d", 'bar_polar(theta="category", r="score")', fig, backend_config["connection_type"])