...
def test_show_displays_native_figure_in_notebook(monkeypatch):
    # ponytail: in a notebook show() must return the native Plotly figure and
    # display it through fig.show() (the working renderer path chosen by
    # setup_plotly_renderer); the kernel renders whatever the renderer emits.
...
def test_plotly_renderer_is_browser_outside_notebooks(monkeypatch):
    # ponytail: notebook/colab detection is env-var based; without those env
    # vars (normal test/CI environment) the renderer must be plain "browser".
    from memframe.utils.plot_renderer import setup_plotly_renderer

    for var in ("COLAB_GPU", "VSCODE_PID", "JPY_PARENT_PID"):
        monkeypatch.delenv(var, raising=False)

    assert setup_plotly_renderer() == "browser"
