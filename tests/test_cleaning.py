...
def get_materialized_table_columns(ctx: Any, result: Dict[str, Any]) -> List[str]:
    """Return columns from the physical table generated by an operation."""
    table = result.get("new_table")
    if not table:
        raise AssertionError(f"Operation result did not include a generated table: {result}")
